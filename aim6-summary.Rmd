...
```{r}
wrapper_calc_sys_sepsis_0(data) |>
...
This record is missing from the 'Syndrome Adjudication' event entirely:
```{r}
data |>
  distinct(record_id) |>
  left_join(data_with_sepsis, by = 'record_id') |>
  filter(is.na(sys_sepsis_0) & is.na(sepsis_present) & is.na(sepsis_clinical_judgement)) |>
  select(
    record_id,
    sepsis_present,
    sepsis_clinical_judgement
...
```{r}
wrapper_calc_sys_septic_shock_0(data) |>
...
```{r}
wrapper_calc_str_septic_shock_0(data) |>
