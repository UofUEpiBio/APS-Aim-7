---
title: "APS 6 Derived Variables Summary"
output:
  github_document:
    toc: true
---
...
The record `52-0023` was erroneously included in the `N=500` data set. As it will be omitted in the final data set, I have filtered it out of the data before deriving the variables below. Consequently, the total counts for any given variable will sum to 499 not 500. This should be corrected with the final `N=500` data set, assuming a different record is chosen to replace `52-0023`.
...
```{r}
missing_sys_hypotension_sev_0(data) |>
  gt()
```
...
```{r}
missing_sofa_total_day_m1(data) |>
  gt()
```
...
These records have missing antimicrobial or pathogen data. These should be queried.

```{r}
data_with_sepsis <- wrapper_calc_sys_sepsis_0(data, dictionary)

data_missing_antimicrobials <- data |>
  filter(event_label == 'Daily In-Hospital Forms') |>
  select(
    record_id,
    daily_antibiotics_0,
...
data_missing_antimicrobials |>
  filter(
    is.na(daily_antibiotics_0) |
    is.na(daily_antiviral_0) |
    (daily_antifungal_0 == 'Administered' &
      is.na(antifungal_agents_0___1) &
      is.na(antifungal_agents_0___2) &
      is.na(antifungal_agents_0___3) &
      is.na(antifungal_agents_0___6) &
      is.na(antifungal_agents_0___7) &
      is.na(antifungal_agents_0___9) &
      is.na(antifungal_agents_0___13))
  ) |>
  select(
    record_id,
    daily_antibiotics_0,
...
These records are missing from the 'Daily In-Hospital Forms' event entirely:
```{r}
data |>
  distinct(record_id) |>
  left_join(data_missing_antimicrobials, by = 'record_id') |>
  filter(
    is.na(daily_antibiotics_0) &
    is.na(daily_antiviral_0) &
...
```{r}
wrapper_calc_str_adrenal_insufficiency_0(data) |>
  display_grand_total(str_adrenal_insufficiency_0)
```
