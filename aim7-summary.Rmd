...
These records were missing (NA) because they were missing all three `daily_resp_rate_8a_*` variables. Some of these have valid branching logic, while others have invalid branching logic.
...
These records were missing (NA). Most of these are due to valid branching logic (daily_bun was 'Not Collected'), but a few records were missing all `daily_bun_nc_*`/`daily_bun_8a_*` pairs, which violates the branching logic (should be mutually exclusive).
...
These records were missing (NA) because they were missing all three `daily_ph_lowest_*` variables. Some of these have valid branching logic, while others have invalid branching logic.
...
These records were missing (NA) `lowtemp_vsorres`. One record is the one missing from the N=500 dataset (`52-0023`) and doesn't need to be queried. The other has `vs_perf == 'No'` so the missing `lowtemp_vsorres` is valid branching logic, but shouldn't it be here?
