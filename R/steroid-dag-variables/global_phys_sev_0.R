...
check_missing_sys_global_phys_sev_0 <- function(data, record_ids) {
  # Check Day 0 parameters
  day0_missing <- data |>
    filter(record_id %in% record_ids, event_label == 'Day 0') |>
    select(record_id, sofa_base_renal_dysnfx, sofa_base_renal_chronic) |>
    distinct() |>
    rowwise() |>
    mutate(missing_params = {
      missing <- c()
      if (is.na(sofa_base_renal_dysnfx)) missing <- c(missing, "sofa_base_renal_dysnfx")
      if (is.na(sofa_base_renal_chronic)) missing <- c(missing, "sofa_base_renal_chronic")
  ...
  # Check Daily In-Hospital Forms parameters (APS variables for days 0, m1, m2)
  daily_missing <- data |>
    filter(record_id %in% record_ids, event_label == 'Daily In-Hospital Forms') |>
    select(record_id, starts_with("hightemp_vsorres_"), starts_with("lowtemp_vsorres_"),
           starts_with("lowsysbp_vsorres_"), starts_with("lowdbp_vsorres_"),
           starts_with("highhr_vsorres_"), starts_with("lowhr_vsorres_"),
           starts_with("highrr_vsorres_"), starts_with("lowrr_vsorres_"),
           starts_with("daily_pa02_lowest_"), starts_with("daily_ph_lowest_"),
           starts_with("daily_sodium_8a_"), starts_with("daily_potassium_8a_"),
           starts_with("daily_cr_8a_"), starts_with("daily_hct_8a_"),
           starts_with("daily_wbc_8a_"), starts_with("daily_gcs_8a_")) |>
    distinct() |>
    ...
    mutate(missing_params = {
      ...
      for (col in c("hightemp_vsorres_0", "lowtemp_vsorres_0", "lowsysbp_vsorres_0",
                    "highhr_vsorres_0", "highrr_vsorres_0", "daily_pa02_lowest_0",
                    "daily_sodium_8a_0", "daily_cr_8a_0", "daily_gcs_8a_0")) {
