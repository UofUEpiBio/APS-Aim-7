...
# Returns a data frame with record_id and str_dah_0 columns (one row per record_id)
wrapper_calc_str_dah_0 <- function(data, dictionary) {
  data |>
    # Ensure one row per record_id (even if data is missing)
    distinct(record_id) |>

    left_join(
      # Calculate str_dah_0 and join back to record_id
      data |>
        ...
        mutate(str_dah_0 = calc_str_dah_0(
          organ_dysfnx_cause_code = organ_dysfnx_cause_code
        )) |>
        select(record_id, str_dah_0),
      by = 'record_id'
    )
}
...
check_missing_str_dah_0 <- function(data, record_ids) {
  data |>
