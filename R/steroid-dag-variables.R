...
# - Returns a single data frame with one row per record_id
get_all_steroid_dag_variables <- function(data, dictionary) {
    data |>
...
# - Returns a data frame with one row for each record_id that has at least one missing value, along with the list of which variables are missing
