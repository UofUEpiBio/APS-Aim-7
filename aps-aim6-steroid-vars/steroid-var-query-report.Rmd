...
In deriving the steroid DAG variables for APS Aim 6, several records could not be properly classified for one or more variables due to missing (NA) or unknown values. This report lists the records that should be queried and the specific data values that need to be queried for each record.

I've organized the report into four general categories:

1. **Missing with Invalid Branching Logic:** The missing data for these records violate branching logic rules. For example, a record might have `trx_0 == 'Available'`, but be missing `daily_paralysis_0` even though the branching logic states it should be present.
2. **Missing Critical Values:** The missing data for these records technically satisfy branching logic rules (i.e., they contain "valid" responses), however, the data in question is considered so integral to the APS project, that it should be present for *every* record included in the N=500 dataset.
3. **Unknown Values:** The data for these records also typically satisfies the branching logic (i.e., "Unknown" was a valid option), however we may want to query some of these values, such as those associated with particularly important DAG variables. For those that cannot be corrected, the steroid variable functions may need to be updated to properly assign "Unknown" a variable value.
4. **Additional Queries:** Queries that don't fall under one of the above three options.

Alongside the records and data values to query, I've included a list of impacted DAG variables. In most cases, if a record was missing or unknown for a relevant value, that record was likewise assigned missing or unknown for the associated variable. However, some variable definitions instead impute "normal" for missing/unknown values (e.g., SOFA scores), which means a record we want to query might instead be classified incorrectly. Further, some data points can be handled in the variable definition, but seem to indicate error during data collection that should be corrected.

**Note:** This file should be run from the root folder of the APS-Code-Sharing repo to properly source the DAG variables in `steroid-dag-variables.R`.
...
We perform this filtering before compiling reports on the Steroid DAG Variables, including this query report.

## Missing Values to Query

These records are missing (NA) for one or more DAG variables:

```{r, echo = FALSE}
records_with_missing <- get_records_with_missing_values(steroid_dag_data)
records_with_missing |>
  gt() |>
  cols_label(missing_cols = "Missing DAG Variables")
```

This list is not a 1:1 match for the records detailed in this report, since some records were likely incorrectly classified, but I included it here to demonstrate that there are a relatively low number of records to query, with several "repeat offenders" in the list.

### Missing with Invalid Branching Logic
...
- 11-0059:
  - `cxr_dm1_available`, `cxr_dm1_opacity`, `ct_dm1_available`, `ct_dm1_opacity`
    - **Violated Branching Logic:**: All are missing, when at least one should be present. The minimum is `cxr_dm1_opacity == 'No relevant imaging completed on this day'`.
    - **Impacted DAG Variables:** Cannot calculate SCAP Criterion Bilateral Opacities DAG
  - `cxr_d0_available`, `ct_d0_available`, `ct_d0_opacity`
    - **Violated Branching Logic:**: `cxr_d0_opacity == 'None'` but all others are missing for that day. If Day 0 imaging data is not available, then `cxr_d0_opacity` should be `'No relevant imaging completed on this day'` instead.
    - **Impacted DAG Variables:** Cannot calculate SCAP Criterion Bilateral Opacities DAG
- 13-0016 and 63-0008:
  - Paired values (`nc` means "Not Collected", `8a` means 8am value):
    ...
    - **Impacted DAG Variables:** Cannot calculate Serum Sodium, Serum Potassium, Serum Creatinine, Hematocrit, White Blood Cell Count components of APS score for APACHE II (Global Physiology Severity DAG); cannot calculate Inflammatory Profile DAGs; cannot calculate SYSTEMATIC Hyperglycemia DAG Variable 2; possibly incorrect value for STREAMLINED Hyperglycemia DAG; cannot calculate SCAP Criterion BUN Value DAG; cannot calculate SCAP Criterion Leukopenia DAG; cannot calculate SCAP Criterion Thrombocytopenia DAG;
- 13-0016:
  - `daily_vasopressors_0___* == 'Unchecked'` (all vasopressor variables for Day 0)
    ...
    - **Impacted DAG Variables:** Cannot calculate Hypotension Severity DAGs
...
- 31-0027:
  - `daily_paralysis_0`, `daily_antifungal_0`, `daily_antiviral_0`, `daily_antibiotics_0`
    - **Violated Branching Logic:** `trx_0 == 'Available'` for this record means values should be present
    - **Impacted DAG Variables:** Cannot calculate Neuromuscular Blockade DAGs, Active Fungal Infections DAGs, Presence of Sepsis Syndrome DAG, Septic Shock DAGs
- 33-0018:
  - `lowsysbp_vsorres`, `lowdbp_vsorres`
    - **Violated Branching Logic:** `vs_perf == 'Yes'` for this record means values should be present
    - **Impacted DAG Variables:** Cannot calculate MAP component of APS score for APACHE II (Global Physiology Severity DAG)
- 45-0003
  - `trx_0`, `daily_paralysis_0`, `daily_antifungal_0`, `daily_antiviral_0`, `daily_antibiotics_0`, `daily_blood_product_0`, `blood_prbc_units_0`
    - **Violated Branching Logic:** `trx_0` should never be missing (also patient has `trx_m1 == 'Available'`, indicating patient was in hospital at Day -1), and if it is `'Available'`, then the other values should be present.
    - **Impacted DAG Variables:** Cannot calculate Neuromuscular Blockade DAGs, Active Fungal Infections DAGs, SYSTEMATIC Presence of Sepsis Syndrome DAG, Septic Shock DAGs, Gastrointestinal Bleeding DAG,
- 45-0011:
  - `daily_resp_rate_8a_0`
    ...
    - **Impacted DAG Variables:** Cannot calculate SCAP Criterion Respiratory Rate
...
- 45-0015:
  - `daily_imv_mode_8a_0`, `daily_epap_8a_0`, `daily_resp_rate_8a_0`/`daily_resp_rate_8a_m1`
    ...
    - **Impacted DAG Variables:** Cannot calculate Respiratory Failure Severity DAGs, SCAP Criterion Respiratory Rate
- 61-0040:
  - `daily_niv_fi02_8a_0`
    - **Violated Branching Logic:** `daily_resp_8a_0 == 'Non-invasive ventilation'` for this record means these values should be present
    - **Impacted DAG Variables:** Cannot calculate Respiratory Failure Severity DAGs
...
- 12-0001, 42-0011, 54-0012, 61-0036:
  - `daily_gluc_nc_0 == 'Not Collected'`: Indicates Day 0 glucose wasn't collected. Technically satisfies branching logic.
    ...
    - **Impacted DAG Variables:** Cannot calculate SYSTEMATIC Hyperglycemia DAG Variable 2
- 13-0016:
  - No data for `event_label == 'Syndrome Adjudication'` (missing entirely from this event)
    - **Impacted DAG Variables:** Cannot calculate Acute Allergic Reaction, Diffuse Alveolar Hemorrhage, Acute Inflammatory Drug Toxicity, and Acute Cryptogenic Organizing Pneumonia DAGs (missing `organ_dysnfx_cause`); cannot calculate SCAP Criterion Bilateral Opacities DAG (missing imaging data);
- 33-0007, 33-0012, 63-0008:
  - `trx_0 == 'Not Available'`: Indicates Day 0 treatments weren't collected, but they should be. Technically satisfies branching logic.
    ...
    - **Impacted DAG Variables:** Cannot calculate Neuromuscular Blockade DAGs, SYSTEMATIC Presence of Sepsis Syndrome DAG, Septic Shock DAGs
- 33-0015:
  - `vs_perf == 'No'`: Indicates Day 0 vital signs weren't collected, but they should be. Technically satisfies branching logic.
    ...
    - **Impacted DAG Variables:** Cannot calculate Temperature, MAP, Heart Rate, and Respiratory Rate components of APS score for APACHE II (Global Physiology Severity DAG); cannot calculate SCAP Criterion Hypothermia DAG;
- 43-0008, 61-0002, 61-0003, 61-0004, 61-0005, 61-0006, 61-0007, 61-0009, 61-0010, 61-0011, 61-0014, 61-0016:
  - `daily_bun_nc_0 == 'Not Collected'`: Indicates Day 0 BUN wasn't collected, but it should be. Technically satisfies branching logic.
    ...
    - **Impacted DAG Variables:** Cannot calculate SCAP Criterion BUN Value DAG
...
- 45-0001:
  - `daily_hct_nc_0 == 'Not Collected'` (same for `daily_hct_nc_m1` and `daily_hct_nc_m2`): Indicates Day 0 hematocrit wasn't collected, but it should be. Technically satisfies branching logic.
    ...
    - **Impacted DAG Variables:** Cannot calculate Hematocrit component of APS score for APACHE II (Global Physiology Severity DAG)
- 63-0008:
  - `dailysofa_perf_0 == 'Not Available'`: Indicates Day 0 SOFA components weren't collected, but they should be. Technically satisfies branching logic.
    ...
    - **Impacted DAG Variables:** Cannot calculate Hypotension Severity DAGs, Respiratory Failure Severity DAGs
...
- 12-0006, 12-0011, 31-0025, 31-0047, 31-0052, 31-0055, 45-0002:
  - `m_endocrine == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for Definite Adrenal Insufficiency DAGs; SYSTEMATIC Hyperglycemia DAG Variable 1;
...
- 12-0006, 12-0011, 31-0017, 31-0052, 33-0016, 41-0003, 45-0004, 54-0035:
  - `m_immunosuppression == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for SYSTEMATIC Definite Adrenal Insufficiency DAG, STREAMLINED Chronic Steroids High Dose DAG, Chronic Immunocompromise DAG,
...
- 12-0006, 12-0011, 24-0006, 24-0022, 31-0022, 45-0001, 45-0004, 54-0035, 61-0002, 61-0041, 64-0008, 64-0018:
  - `mhccster == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for Chronic Moderate-Dose Steroid Use DAGs (61-0002 is only "Unknown" for STREAMLINED version)
...
- 12-0006, 12-0011, 31-0047, 31-0052, 31-0055, 45-0004, 45-0005, 61-0022, 61-0038:
  - `mhrheumd == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for SYSTEMATIC Steroid Responsive Comorbidity DAG
      - For 12-0006, 12-0011, 31-0052, 45-0004 ONLY, assigned "99 (Unknown)" for STREAMLINED Steroid Responsive Comorbidity DAG, Active Vasculitis DAG
- 12-0006, 12-0011, 31-0025, 31-0047, 31-0055, 41-0003, 42-0012, 45-0002, 45-0004, 45-0005:
  - `m_pulmonary == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for SYSTEMATIC Steroid Responsive Comorbidity DAG, Obstructive Lung Disease DAGs
- 12-0006, 12-0011, 24-0006, 24-0022, 31-0022, 33-0016, 45-0004, 54-0035, 61-0002, 61-0041, 64-0008, 64-0018:
  - `mhantifungals == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for Active Fungal Infections DAGs
- 12-0006, 12-0011, 31-0030, 31-0047, 31-0052, 31-0055, 41-0003, 45-0004, 45-0005:
  - `m_neurologic == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for STREAMLINED Dementia DAG, Chronic Neuromuscular Disease DAG,
- 45-0014, 54-0003, 63-0004:
  - `daily_resp_8a_0 == 'Unknown'`
    - **Impacted DAG Variables:** Assigned "99 (Unknown)" for Variable 1 "Type of Support" under Respiratory Failure Severity DAGs, prevents calculation of Variable 2 "S/F Ratio Closest to 8am" under the same DAGs.


## Additional Queries

- 61-0002, 61-0004, 61-0012, 61-0022, 61-0038, 61-0044, 61-0045, 61-0049, 61-0051, 61-0052, 61-0058, 61-0063:
  - These records have an unusually high packed red blood cell unit count (300+ when it should most often be less than 5) and should be queried. As they are from the same site, we suspect that the person entering the data simply misinterpreted the question and didn't answer in "units of blood" but in some other unit.
  - **Values to Query:**: `blood_prbc_units_0`, `blood_prbc_units_m1`, `blood_prbc_units_m2`
  - **Impacted DAG Variables:** Invalid value for Gastrointestinal Bleeding DAG
