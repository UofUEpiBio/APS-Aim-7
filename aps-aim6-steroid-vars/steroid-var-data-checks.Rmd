---
title: "APS Aim 6 Steroid DAG - Data Checks"
output:
  github_document:
    toc: true
---
...
## UPDATE with relative filepath to .RData file on your machine
...
While the file `steroid-var-query-report.Rmd` contains a report on what data needs to be queried, this file contains the *code* used to perform the data checks that lead to that report, specifically identifying missing, unknown, or unusual values for each steroid DAG. Code in this file was also used to verify that the final variable values for the steroid DAG variables were correct by examining the results for unusual patterns and distribution of values.

If a given variable does not include a sub-header for "missing" or "unknown" values, then no additional data checks were necessary for that variable.

**Note 1:** This file should be run from the root folder of the APS-Code-Sharing repo to properly source the DAG variables in `steroid-dag-variables.R`.

**Note 2:** This was the primary working document during the development of the steroid DAG variable functions. As such, it is not very polished and sometimes in-consistently formatted. We HIGHLY recommend using the Query Report document instead of this document, where the data is better organized. This document is meant more as a reference for the code and process used to investigate the DAG variable data.

### Reminder about Excluded Record

As described in `steroid-var-query-report.Rmd`, the record `52-0023` was excluded from the data before running these data checks.
...
These records couldn't be scored because they are missing (NA) both `hightemp_vsorres` and `lowtemp_vsorres`. The record has `vs_perf == 'No'` (Day 0 vital signs weren't collected), so missing `hightemp_vsorres` and `lowtemp_vsorres` technically satisfies the branching logic, but these should probably have values.
...
These records couldn't be scored because they are missing (NA) both `lowsysbp_vsorres` and `lowdbp_vsorres`. One has `vs_perf == 'No'` (Day 0 vital signs weren't collected), so missing `lowsysbp_vsorres` and `lowdbp_vsorres` technically satisfies the branching logic, but these should probably have values. This is the same record missing measurements for Temperature (above). The other has `vs_perf == 'Yes'`, so for that record those missing values violate the branching logic.
...
These records couldn't be scored because they are missing (NA) both `highrr_vsorres` and `lowrr_vsorres`.The record has `vs_perf == 'No'` (Day 0 vital signs weren't collected), so missing `highrr_vsorres` and `lowrr_vsorres` technically satisfies the branching logic, but these should probably have values. This is the same record missing measurements for Temperature (above).
...
These records couldn't be scored because they are missing (NA) all pairs for `daily_na_nc_*`/`daily_na_8a_*`. The branching logic dictates these should be mutually exclusive - if one is NA, the other shouldn't be. These should be queried.
...
These records couldn't be scored because they are missing (NA) all pairs for `daily_k_nc_*`/`daily_k_8a_*`. The branching logic dictates these should be mutually exclusive - if one is NA, the other shouldn't be. These are the same records missing Serum Sodium (above) and should be queried.
...
These records couldn't be scored because they are missing (NA) all pairs for `daily_cr_nc_*`/`daily_cr_8a_*`. The branching logic dictates these should be mutually exclusive - if one is NA, the other shouldn't be. These are the same records missing Serum Sodium (above) and should be queried.
...
These records couldn't be scored. Three are missing (NA) all pairs for `daily_hct_nc_*`/`daily_hct_8a_*`. The branching logic dictates these should be mutually exclusive - if one is NA, the other shouldn't be. These are the same records missing Serum Sodium (above) and should be queried.
...
These records couldn't be scored because they are missing (NA) all pairs for `daily_wbc_nc_*`/`daily_wbc_8a_*`. The branching logic dictates these should be mutually exclusive - if one is NA, the other shouldn't be. These are the same records missing Serum Sodium (above) and should be queried.
...
```{r}
wrapper_calc_sys_global_phys_sev_0(data, dictionary) |>
  display_grand_total(sys_global_phys_sev_0)
```
...
## Hypotension Severity -- Systematic & Streamlined DAG
...
```{r}
wrapper_calc_sys_hypotension_sev_0(data) |>
  display_grand_total(sys_hypotension_sev_0)
```

#### Unknown/Missing Values

The two records missing for this variable are missing almost all of their labs. These should be queried.
...
```{r}
wrapper_calc_sys_nmblockade_0(data) |>
  display_grand_total(sys_nmblockade_0)
```
...
Early in the process we ran these records by Matt and received this response:
...
These records were missing `daily_paralysis_0`, but this was considered valid (e.g., "Not Applicable") by the branching logic of `trx_0 == 'Not Available'`. Thus, they were set to `sys_nmblockade_0 = 0` according to the variable definition. However, as `trx_0` should be Available, we've included these records.
...
```{r}
wrapper_calc_sys_inflamprofile_0(data) |>
  pivot_longer(
...
```{r}
wrapper_calc_str_inflamprofile_0(data) |>
  display_grand_total(str_inflamprofile_0)
```

### Missing Values

The records missing (NA) for the `crp` component variable of "Inflammatory Profile" (Systematic DAG) above are also missing for this variable.
...
```{r}
wrapper_calc_sys_resp_failure_sev_0(data, dictionary) |>
  display_grand_total(resp_support_type_0)
```
...
```{r}
wrapper_calc_sys_def_adrenal_insufficiency_0(data) |>
  display_grand_total(sys_def_adrenal_insufficiency_0)
```
...
```{r}
wrapper_calc_str_def_adrenal_insufficiency_0(data) |>
  display_grand_total(str_def_adrenal_insufficiency_0)
```

### Unknown and Missing Values

Those with "Unknown" for `m_endocrine` listed under the variable "Definite Adrenal Insufficiency" (Systematic DAG) are also "Unknown" for this variable.
...
```{r}
wrapper_calc_sys_chron_steroid_moddose_0(data) |>
  display_grand_total(sys_chron_steroid_moddose_0)
```
...
```{r}
wrapper_calc_str_chron_steroid_moddose_0(data) |>
  display_grand_total(str_chron_steroid_moddose_0)
```
...
```{r}
wrapper_calc_str_chron_steroid_highdose_0(data) |>
  display_grand_total(str_chron_steroid_highdose_0)
```
...
```{r}
wrapper_calc_sys_steroid_comorb_0(data) |>
  display_grand_total(sys_steroid_comorb_0)
```
...
```{r}
wrapper_calc_str_steroid_comorb_0(data) |>
  display_grand_total(str_steroid_comorb_0)
```
...
```{r}
wrapper_calc_sys_obstruct_lung_0(data) |>
  display_grand_total(sys_obstruct_lung_0)
```
...
```{r}
wrapper_calc_sys_active_covid19_0(data, dictionary) |>
...
```{r}
wrapper_calc_sys_chron_immunocomp_0(data) |>
  display_grand_total(sys_chron_immunocomp_0)
```
...
```{r}
wrapper_calc_sys_hyperglycemia_0(data) |>
  display_grand_total(sys_hyperglyc_hist_0)
```
...
```{r}
wrapper_calc_str_hyperglyc_hist_0(data) |>
  display_grand_total(str_hyperglyc_hist_0)
```
...
```{r}
wrapper_calc_sys_active_influenza_0(data, dictionary) |>
  display_grand_total(sys_active_influenza_0)
```
...
```{r}
wrapper_calc_sys_baseline_performance_status_0(data, dictionary) |>
  display_grand_total(sys_frailty_status_0)
```
...
The Charlson Comorbidity Index (Reference: https://pubmed.ncbi.nlm.nih.gov/34991091/).
...
wrapper_calc_sys_baseline_performance_status_0(data, dictionary) |>
  summarize(
    n = n(),
    na_count = sum(is.na(sys_comorbid_burden_0)),
...
```{r}
wrapper_calc_sys_organ_failure_trajectory(data) |>
  display_grand_total(sofa_total_day_0)
```
...
```{r}
wrapper_calc_sys_organ_failure_trajectory(data) |>
  display_grand_total(sofa_total_day_m1)
```
...
```{r}
wrapper_calc_sys_delirium_0(data) |>
  display_grand_total(sys_delirium_0)
```
...
```{r}
wrapper_calc_sys_active_fungal_0(data) |>
  display_grand_total(sys_active_fungal_0)
```
...
```{r}
wrapper_calc_sys_sepsis_0(data, dictionary) |>
  display_grand_total(sys_sepsis_0)
```

### Unknown/Missing Values

These records have missing antimicrobial data. These should be queried.

```{r}
data |>
  filter(event_label == 'Daily In-Hospital Forms') |>
  filter(
    is.na(daily_antibiotics_0) |
    is.na(daily_antiviral_0) |
    is.na(daily_antifungal_0)
  ) |>
  select(
    record_id,
    trx_0,
...
## (AWAITING REWORKED DEFINITION) Presence of ARDS Syndrome -- Systematic & Streamlined DAG
...
```{r}
wrapper_calc_sys_ards_0(data) |>
  display_grand_total(sys_ards_0)
```

### Unknown/Missing Values

The missing (NA) record is `13-0016`, which is missing from the 'Syndrome Adjudication' event entirely (see Presence of Sepsis Syndrome DAGs above).

## (AWAITING REWORKED DEFINITION) Presence of Pneumonia Syndrome -- Systematic & Streamlined DAG
...
```{r}
wrapper_calc_sys_pneumonia_0(data) |>
  display_grand_total(sys_pneumonia_0)
```
...
```{r}
wrapper_calc_sys_septic_shock_0(data, dictionary) |>
  display_grand_total(sys_septic_shock_0)
```
...
```{r}
wrapper_calc_str_septic_shock_0(data, dictionary) |>
  display_grand_total(str_septic_shock_0)
```
...
```{r}
wrapper_calc_str_gi_bleeding_0(data) |>
...
These records are missing (NA) due to having `trx_0 == NA` (though `trx_m1 == 'Available'` and `trx_m2 == 'Not Available'`). This shouldn't happen and this record should be queried.
...
Additionally, these records have an unusually high unit count and should be queried. As they are from the same site, we suspect that the person entering the data simply misinterpreted the question and didn't answer in "units of blood" but in some other unit.

```{r}
data_with_gi_bleeding |>
  filter(str_gi_bleeding_0 > 6) |>
  select(record_id, str_gi_bleeding_0, blood_prbc_units_0, blood_prbc_units_m1, blood_prbc_units_m2) |>
  gt()
```
...
```{r}
wrapper_calc_str_major_surgery_0(data) |>
  display_grand_total(str_major_surgery_0)
```
...
```{r}
wrapper_calc_str_dementia_0(data) |>
  display_grand_total(str_dementia_0)
```
...
```{r}
wrapper_calc_str_acute_allergic_reaction_0(data, dictionary) |>
  display_grand_total(str_acute_allergic_reaction_0)
```
...
```{r}
wrapper_calc_str_dah_0(data, dictionary) |>
  display_grand_total(str_dah_0)
```
...
```{r}
wrapper_calc_str_drug_toxicity_0(data, dictionary) |>
  display_grand_total(str_drug_toxicity_0)
```
...
```{r}
wrapper_calc_str_cop_0(data, dictionary) |>
  display_grand_total(str_cop_0)
```
...
```{r}
wrapper_calc_str_scap_rr_0(data, dictionary) |>
  display_grand_total(str_scap_rr_0)
```
...
- **Day -1:** Record `45-0015` has `daily_resp_8a_0 == 3 (IMV without ECMO)`, but with the same problems as Day 0. However, record `45-0011` has `daily_resp_8a_m1 == 6 (Standard flow)`. According to the branching logic, this record should then have a value for `daily_standard_rr_8a_m1`, but it is missing.
...
```{r}
wrapper_calc_str_scap_bun_0(data) |>
  display_grand_total(str_scap_bun_0)
```
...
```{r}
wrapper_calc_str_scap_acidosis_0(data) |>
  display_grand_total(str_scap_acidosis_0)
```
...
```{r}
wrapper_calc_str_scap_bilateral_opacities_0(data, dictionary) |>
  display_grand_total(str_scap_bilateral_opacities_0)
```
...
```{r}
wrapper_calc_str_scap_leukopenia_0(data) |>
  display_grand_total(str_scap_leukopenia_0)
```
...
```{r}
wrapper_calc_str_scap_thrombocytopenia_0(data) |>
  display_grand_total(str_scap_thrombocytopenia_0)
```
...
```{r}
wrapper_calc_str_scap_hypothermia_0(data) |>
  display_grand_total(str_scap_hypothermia_0)
```

### Unknown/Missing Values

This record was missing (NA) `lowtemp_vsorres`. It has `vs_perf == 'No'`, so the missing `lowtemp_vsorres` is valid branching logic, but since all patients should have their Day 0 vital signs, this should be queried.
...
```{r}
wrapper_calc_str_neuromuscular_disease_0(data) |>
  display_grand_total(str_neuromuscular_disease_0)
```
...
## (NEW, NEEDS REVIEW) PSI Score -- Streamlined DAG
...
```{r}
wrapper_calc_str_psi_score_0(data) |>
...
```{r}
wrapper_calc_str_vasculitis_0(data) |>
  display_grand_total(str_vasculitis_0)
```
